...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signUp = useCallback(async (email: string, password: string, role: UserRole, fullName: string, phone: string) => {
    ...
    if (users.some((item) => item.email.toLowerCase() === email.toLowerCase())) return { error: 'این ایمیل قبلاً ثبت شده است.' };
    const now = new Date().toISOString();
    const user: DemoUser = { id: crypto.randomUUID(), email, password, role, full_name: fullName,
      phone: phone || null, avatar_url: null, status: 'active', city: null, created_at: now, updated_at: now };
    writeUsers([...users, user]); localStorage.setItem(CURRENT_KEY, user.id);
    setProfile(user); setSession(createSession(user)); return { error: null };
  }, []);

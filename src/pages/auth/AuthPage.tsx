...
import { Truck, Package, Shield, ArrowLeft } from 'lucide-react';
...
export function AuthPage({ mode, onModeChange }: AuthPageProps) {
  ...
  const validate = (): boolean => {
    ...
    if (mode === 'register') {
      ...
      if (phone && !/^09\d{9}$/.test(phone)) errs.phone = 'شماره موبایل معتبر نیست (مثال: 09123456789)';
    }
    setErrors(errs);
      ...
      <div className="flex-1 flex flex-col justify-center px-6 py-8 max-w-md mx-auto w-full">
        ...
        <div className="flex gap-1 bg-gray-100 rounded-xl p-1 mb-6">
          ...
          <button
            ...
            className={cn(
              'flex-1 py-2.5 text-sm font-medium rounded-lg transition-all',
              mode === 'register' ? 'bg-white text-primary-700 shadow-sm' : 'text-gray-500'
            )}
          >
            ثبت‌نام
          </button>
        ...
        {mode === 'login' && (
          <p className="text-center text-sm text-gray-400 mt-6">
            حساب ندارید؟{' '}
            <button onClick={() => onModeChange('register')} className="text-primary-600 font-medium">
              ثبت‌نام کنید
            </button>
          </p>
        )}
        ...
        <div className="mt-8 flex items-center justify-center gap-2 text-xs text-gray-300">
          ...
          <span>امنیت اطلاعات شما تضمین می‌شود</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      <div className={cn(
        ...
        active ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-400'
      )}>
        {icon}
      </div>
